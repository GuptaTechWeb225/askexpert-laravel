...
function loadVendorCaptcha() {
    let link = $('.get-login-recaptcha-verify').data('link');
    if (link) {
        $('#default_recaptcha_id').attr(
            'src',
            link + "/" + Math.random() + '?captcha_session_id=restaurantRecaptchaSessionKey'
        );
    }
}
...
$('.submit-login-form').on('click',function (){
    ...
    try{
     ...
     if (response.length === 0) {
        ...
        // ✅ Hidden input add/update
        if ($("#g-recaptcha-response-hidden").length === 0) {
            $('<input>').attr({
                type: 'hidden',
            ...
            $("#g-recaptcha-response-hidden").val(response);
        }
        $.ajaxSetup({
            headers: {
...
$('.get-login-recaptcha-verify').on('click', function () {
    document.getElementById('default_recaptcha_id').src = $(this).data('link') + "/" + Math.random()+'?captcha_session_id=restaurantRecaptchaSessionKey';
});
