            ...
            <div class="col-md-6">
                <h4 class="d-flex align-items-center text-capitalize gap-10 mb-0">
                    <img src="{{dynamicAsset(path: 'public/assets/back-end/img/order-statistics.png')}}"
                        alt="">
                    {{translate('Top Performing Restaurant')}}
                </h4>
            </div>
...
<span id="order-statistics-data"
    data-customer-text="{{translate('Customers')}}"
    data-rewards-text="{{translate('Rewards Given')}}"
    data-customer-data="{{ json_encode($customerData) }}"
    data-rewards-data="{{ json_encode($rewardsData) }}"
    data-label="{{ json_encode($label) }}"></span>
