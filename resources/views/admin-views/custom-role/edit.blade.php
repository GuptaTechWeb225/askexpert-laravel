            ...
            <div class="card-body">
                <form id="submit-create-role" action="{{route('admin.custom-role.update',[$role['id']])}}" method="post" class="text-start">
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-3 col-sm-6">
                            <div class="form-group form-check">
                                <input type="checkbox" class="form-check-input module-permission" name="modules[]"
                                       {{in_array('user_management',(array)json_decode($role['module_access']))?'checked':''}} value="user_management"
                                       id="user_management">
                        ...
                        <div class="col-lg-3 col-sm-6">
                            <div class="form-group form-check">
                                <input type="checkbox" name="modules[]" value="restaurant_management"
                                       class="form-check-input module-permission"
                                       id="restaurant" {{in_array('restaurant_management',(array)json_decode($role['module_access']))?'checked':''}}>
                                <label class="form-check-label text-capitalize"
                                       style="{{$direction === "rtl" ? 'margin-right: 1.25rem;' : ''}};"
                                       for="restaurant">{{translate('restaurant_management')}}</label>
                            </div>
                        </div>
                        <div class="col-lg-3 col-sm-6">
                            <div class="form-group form-check">
                                <input type="checkbox" name="modules[]" value="plan_management"
                                       class="form-check-input module-permission"
                                       id="plan" {{in_array('plan_management',(array)json_decode($role['module_access']))?'checked':''}}>
                                <label class="form-check-label text-capitalize"
                                       style="{{$direction === "rtl" ? 'margin-right: 1.25rem;' : ''}};"
                                       for="plan">{{translate('plan_management')}}</label>
                            </div>
                        </div>
