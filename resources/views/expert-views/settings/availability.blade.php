...
@section('title', translate('Availability Settings'))
...
<div class="content container-fluid">
    ...
    <form action="{{ route('expert.settings.availability.update') }}" method="POST">
        @csrf
        <div class="card">
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover">
                        ...
                        <tbody>
                            ...
                            @php
                            $dayName = ucfirst($avail->day);
                            @endphp
                            <tr>
                                <td class="text-capitalize font-weight-bold">{{ translate($dayName) }}</td>
                                <td>
                                    <input type="time" name="days[{{ $avail->day }}][start]"
                                        value="{{ $avail->start_time ? $avail->start_time->format('H:i') : '09:00' }}"
                                        class="form-control">
                                </td>
                                <td>
                                    <input type="time" name="days[{{ $avail->day }}][end]"
                                        value="{{ $avail->end_time ? $avail->end_time->format('H:i') : '18:00' }}"
                                        class="form-control">
                                </td>
                                <td class="text-center">
                                    <label class="switcher mx-auto">
                                        <input class="switcher_input" type="checkbox"
                                            name="days[{{ $avail->day }}][active]"
                                            value="1" {{ $avail->is_active ? 'checked' : '' }}>
