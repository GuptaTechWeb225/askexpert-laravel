...
if (!function_exists('getStorageImages')) {
    function getStorageImages($path, $type = null, $source = null): string
    {
        if ($source && base_path($source)) {
            ...
            return (is_array($path) && !empty($path) && $path['status'] == 200)
                ? $path['path'] : dynamicAsset($source);
        }
        ...
        if (isset($placeholderMap[$type])) {
            if (is_array($placeholderMap[$type])) {
                ...
                return (is_array($path) && !empty($path) && $path['status'] == 200)
                    ? $path['path'] : $placeholderPath;
            } else {
                return (is_array($path) && !empty($path) && $path['status'] == 200)
                    ? $path['path'] : dynamicAsset(path: 'public/assets/' . $placeholderMap[$type]);
            }
        }

        return (is_array($path) && !empty($path) && $path['status'] == 200)
            ? $path['path'] : dynamicStorage(path: 'public/assets/front-end/img/placeholder/placeholder-2-1.png');
    }
}
