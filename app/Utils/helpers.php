...
class Helpers
{
    ...
    public static function getRestaurantInformation($request = null)
    {
        $restaurant = null;

        // Restaurant guard (session based login)
        if (auth('restaurant')->check()) {
            $restaurant = auth('restaurant')->user();
        }
        // Restaurant Sanctum token based login
        elseif (auth('sanctum')->check()) {
            $user = auth('sanctum')->user();
        ...
        // Request ke through direct user
        elseif (is_object($request) && method_exists($request, 'user')) {
            $user = $request->user();
            if ($user instanceof \App\Models\Restaurant) {
                $restaurant = $user;
    ...
    public static function addon_permission_check($addonName, $restaurantId = null)
    {
        $restaurantId = $restaurantId ?? auth('restaurant')->id();
        ...
        if (!$purchasedPlan || !$purchasedPlan->plan) {
            return false;
        }

        $plan = $purchasedPlan->plan;

        $expiryDate = Carbon::parse($purchasedPlan->created_at)
            ->addDays($plan->plan_duration ?? 0);
        ...
        $addons = is_string($plan->plan_addons)
            ? json_decode($plan->plan_addons, true)
            : ($plan->plan_addons ?? []);
        ...
        // 🛠️ Debug Logs
        Log::info('Checking addon permission:', [
            'restaurant_id' => $restaurantId,
            'plan_addons_raw' => $plan->plan_addons,
            'plan_addons_decoded' => $addons,
            'addonName_requested' => $addonName,
            'addonName_lowercase' => strtolower($addonName),
            'addons_lowercase' => array_map('strtolower', $addons),
        ]);
        // ✅ case-insensitive check
        return in_array(
            strtolower($addonName),
            array_map('strtolower', $addons)
        );
    }
