...
class CustomerAuthController extends Controller
{
    ...
    // GuestController.php
    public function storeReturnUrl(Request $request)
    {
        session()->put('keep_return_url', $request->return_url);

        if ($request->has('pending_question')) {
            session()->put('pending_expert_question', $request->pending_question);
        }
    ...
    public function loginByEmailOrPhone(Request $request): JsonResponse|RedirectResponse
    {
        ...
        if (auth('customer')->attempt([$loginMedia => $user[$loginMedia], 'password' => $request['password']], $remember)) {
            ...
            $redirectUrl = session()->pull('keep_return_url', url('/my/home'));

            if (session()->has('pending_expert_question')) {
                $pendingQuestion = session()->pull('pending_expert_question'); // raw text, no urlencode
                $redirectUrl = route('ask.expert.start'); // POST me bhejne ke liye sirf route
            } else {
                $redirectUrl = session()->pull('keep_return_url', url('/my/home'));
            }
            ...
            if ($request->ajax()) {
                return response()->json([
                    ...
                    'pending_question' => $pendingQuestion ?? null, // POST ke liye raw text
                    'reload' => true,
                ]);
            }
