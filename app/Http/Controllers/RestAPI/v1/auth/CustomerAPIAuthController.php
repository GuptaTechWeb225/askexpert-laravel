...
{
    use CustomerTrait, PushNotificationTrait;
    ...
    public function register(Request $request): JsonResponse
    {
        ...
        if ($request['referral_code']) {
            $refer_user = $this->customerRepo->getFirstWhere(params: [
                'referral_code' => $request['referral_code']
            ]);
        }

        $temporaryToken = Str::random(10);
        $dummyImage = 'user.png';
        ...
        $user = $this->customerRepo->add([
            ...
            'cm_firebase_token' => $request->cm_firebase_token ?? null,
            'loyalty_point' => 0,
        ]);
        ...
        $userData = [
            ...
            'image' => asset('assets/back-end/img/placeholder/' . $user->image), // ✅ correct full URL
            'temporary_token' => $temporaryToken,
            ...
            if (auth()->attempt($data)) {
                ...
                return response()->json([
                    ...
                    'user' => [
                        ...
                        'temporary_token' => $user->temporary_token ?? null,
    ...
    public function passwordResetRequest(Request $request): JsonResponse
    {
        ...
        Log::info('Customer lookup result', ['customer' => $customer]);

        if (isset($customer)) {
            $OTPIntervalTime = getWebConfig(name: 'otp_resend_time') ?? 60;
            $passwordVerificationData = DB::table('password_resets')->where('identity', $request['email'])->first();

            if ($passwordVerificationData) {
                Log::info('Previous password reset entry found', [
                    'created_at' => $passwordVerificationData->created_at,
                    'diff_seconds' => Carbon::parse($passwordVerificationData?->created_at)->DiffInSeconds()
                ]);
            }
            ...
            if (isset($passwordVerificationData) && Carbon::parse($passwordVerificationData?->created_at)->DiffInSeconds() < $OTPIntervalTime) {
                ...
                Log::warning('OTP resend time not yet passed', ['wait_time' => $time]);
            ...
            DB::table('password_resets')->updateOrInsert(['identity' => $request['email']], [
                'token' => $token,
                'created_at' => now(),
            ]);

            Log::info('Password reset entry updated in DB', ['email' => $request['email']]);
            ...
            try {
                ...
                Log::info('Generated reset URL', ['url' => $resetUrl]);
                ...
                if (isset($emailServices['status']) && $emailServices['status'] == 1) {
                    Log::info('Triggering PasswordResetEvent', ['email' => $customer['email']]);
                    event(new PasswordResetEvent(email: $customer['email'], data: $data));
                }
            } catch (\Exception $exception) {
                Log::error('Password reset email sending failed', ['exception' => $exception->getMessage()]);
                return response()->json(['errors' => [
                    ['code' => 'config-missing', 'message' => translate('Email_configuration_issue.')]
                ]], 400);
            }

            Log::info('Password reset email sent successfully', ['email' => $customer['email']]);
    ...
    public function update_cm_firebase_token(Request $request)
    {
        ...
        if ($user == 'offline') {
            ...
            if ($guest) {
                $guest->fcm_token = $request['token'];
                $guest->save();
            } else {
                Log::error("❌ Guest Not Found", ['guest_id' => $request->guest_id]);
            }
        } else {
