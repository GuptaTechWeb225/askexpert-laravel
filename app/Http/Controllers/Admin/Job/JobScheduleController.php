...
class JobScheduleController extends BaseController
{
    ...
    public function getAnalyticsView(Request $request): View|RedirectResponse
    {
        ...
        $retentionDaily = $this->getUserRetentionData('daily');
        $retentionWeekly = $this->getUserRetentionData('weekly');
        $retentionMonthly = $this->getUserRetentionData('monthly');

        $data = self::getOrderStatusData();
        $restaurantGraphData = $this->getChartData();

        $customers = $this->customerRepo->getListWhereBetween(
            searchValue: $request['searchValue'],
            filters: $filters,
            relations: ['lastBillPayment'],
            whereBetween: 'created_at',
            dataLimit: getWebConfig(name: WebConfigKey::PAGINATION_LIMIT),
            appends: $request->all(),
        );

        // 👇 yaha pass kar do view me
        return view(Customer::ANALYTICS[VIEW], compact(
            'data',
            'customers',
            'retentionDaily',
            'retentionWeekly',
    ...
    public function getChartData(): array
    {
        ...
        // ---- Ordered (bill payments se) ----
        $ordersByDay = DB::table('restaurant_bill_payments')
            ->select(DB::raw('DAYNAME(created_at) as day_name'), DB::raw('COUNT(*) as total'))
            ->whereBetween('created_at', [
                now()->startOfWeek(),
                now()->endOfWeek()
            ])
        ...
        foreach ($weekDays as $day) {
            $ordersSeries[] = $ordersByDay[$day] ?? 0;
        }

        // ---- Waitlist (booked, seated, cancelled) ----
        $waitlistStatuses = [
            'visited'   => 'seated',
            'booked'    => 'booked',
            'cancelled' => 'cancelled',
        ...
        foreach ($waitlistStatuses as $key => $status) {
            $data = DB::table('restaurant_waitlists')
                ->select(DB::raw('DAYNAME(created_at) as day_name'), DB::raw('COUNT(*) as total'))
                ->where('status', $status)
                ->whereBetween('created_at', [
                    now()->startOfWeek(),
    ...
    public function getUserRetentionData($type = 'daily')
    {
        $now = Carbon::now();
        ...
        if ($type === 'daily') {
            ...
            $retained = DB::table('users')
                ->whereDate('created_at', $now->toDateString())
                ->whereDate('updated_at', '>', $now->toDateString())
                ->count();

            $percentage = $signups > 0 ? round(($retained / $signups) * 100, 2) : 0;
        ...
        if ($type === 'monthly') {
            // 👉 current month ke din (1 se last day tak)
            $daysInMonth = $now->daysInMonth;

            return collect(range(1, $daysInMonth))->map(function ($day) use ($now) {
