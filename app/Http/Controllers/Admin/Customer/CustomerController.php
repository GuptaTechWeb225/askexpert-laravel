...
class CustomerController extends BaseController
{
    ...
    public function getListView(Request $request): View|RedirectResponse
    {
        ...
        $blockedUsers = User::where('is_active', 0)
            ->orWhere('is_temp_blocked', 1)
            ->count();

        $totalQuestions = ChatSession::count();
    ...
    public function getView(Request $request, $id): View|RedirectResponse
    {
        ...
        $chatSessions = ChatSession::with(['category', 'expert', 'firstMessage'])
            ->where('user_id', $customer->id)
            ->orderByDesc('started_at')
            ->paginate(10); // change per need
