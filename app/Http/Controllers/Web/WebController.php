...
class WebController extends Controller
{
    ...
    public function contact_store(Request $request)
    {
        $request->validate([
            ...
            'subject' => 'required|in:General Inquiry,Contact Inquiry,Career Inquiry',
            'message' => 'required|string|min:10',
        ], [
            'first_name.required' => translate('First name is required'),
            'last_name.required' => translate('Last name is required'),
            'email.required' => translate('Email is required'),
            'email.email' => translate('Please enter a valid email'),
            'mobile_number.required' => translate('Phone number is required'),
            'mobile_number.max' => translate('Phone number may not be greater than 20 characters'),
            'subject.required' => translate('Please select a subject'),
            'message.required' => translate('Message is required'),
            'message.min' => translate('Message must be at least 10 characters'),
        ]);

        // Sanitize phone: remove non-digits
        $cleanPhone = preg_replace('/[^0-9]/', '', $request->mobile_number);
        $phoneLength = strlen($cleanPhone);

        if ($phoneLength < 4 || $phoneLength > 20) {
            return back()->withErrors(['mobile_number' => translate('Phone number must be between 4 and 20 digits')])->withInput();
        }
    ...
    public function pay_offline_method_list(Request $request)
    {
       ...
       public function viewNotifications(Request $request): View
    {
        $data = "";
        return view(VIEW_FILE_NAMES['account_notifications'], compact('data'));
    }
}
